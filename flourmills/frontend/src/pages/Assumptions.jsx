...
import { Button, Card, CardBody, CardDescription, CardHeader, CardTitle, Input, Label } from '../components/ui/Primitives.jsx';
  ...
  return (
    <div>
      ...
      <div className="mt-1 relative">
        <Input id={name} type={type} step={step} disabled={disabled}
               {...register(name, { valueAsNumber: type === 'number' })} />
        {suffix && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-muted pointer-events-none">{suffix}</span>}
      </div>
      {error && <div className="text-xs text-red-600 mt-1">{error.message}</div>}
    </div>
  );
}
...
function Section({ title, description, children }) {
  return (
    <Card>
      ...
      <CardBody>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-4">{children}</div>
      </CardBody>
    </Card>
  );
}

/* Flat schema matching every primitive input in the Excel model */
...
function toFlat(assumption, projectName) {
  ...
  Object.keys(schema.shape).forEach((k) => {
    if (k === 'projectName') return;
    const v = get(assumption, k);
    flat[k] = v ?? (k.startsWith('location.') ? '' : 0);
    if (k === 'dates.modelStartDate' && typeof v === 'string' && v.includes('T')) {
      flat[k] = v.slice(0, 10);
    }
  });
  return flat;
...
function fromFlat(flat, baseAssumption) {
  const out = JSON.parse(JSON.stringify(baseAssumption));
  Object.keys(flat).forEach((k) => {
    if (k === 'projectName') return;
    set(out, k, flat[k]);
  });
  // Also update senior.tenorMonths = tenorYears * 12 (derived convenience)
  if (out.financing?.senior?.tenorYears != null) {
    out.financing.senior.tenorMonths = Number(out.financing.senior.tenorYears) * 12;
  }
  return out;
}
...
export default function Assumptions() {
  ...
  const defaultValues = current ? toFlat(current.assumption, current.projectName) : {};
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      // Save project name separately so it updates the project-level field too
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-xs text-muted uppercase tracking-wider">Inputs</div>
          <h1 className="text-2xl font-semibold text-ink">Assumptions</h1>
          <p className="text-sm text-muted mt-1">
            This is the master input sheet. Every field is editable here.
            Saving invalidates the current run so click <b>Run Model</b> after making changes and saving.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button type="submit" disabled={!isDirty || saveState === 'saving'}>
            <Save size={14} /> {saveState === 'saving' ? 'Saving…' : 'Save Assumptions'}
          </Button>
        </div>
      </div>

      {/* Project */}
      <Section title="Project Identity" description="Name and location">
      ...
      {/* Dates */}
      <Section title="Dates & Macro" description="Development window, construction, FX">
      ...
      {/* System */}
      <Section title="System Sizing" description="Installed capacity">
        ...
        <Field label="Battery (LFP) Capacity"   name="system.batteryCapacity"         register={register} error={errors['system.batteryCapacity']}         suffix="kWh" />
      ...
      {/* Energy */}
      <Section title="Energy Demand" description="Customer counts and daily consumption">
      ...
      {/* Tariff */}
      <Section title="Tariff & Escalation" description="Applied to all billed energy">
      ...
      {/* Technical */}
      <Section title="Technical" description="Uptime × collection = % of sales">
      ...
      {/* Opex */}
      <Section title="Diesel & Generator Opex" description="Annual amounts with their own escalation">
      ...
      <Section title="Other Opex (as % of Capex)" description="Annual base amounts computed from total capex">
      ...
      {/* Depreciation lives per asset class */}
      <Section title="Depreciation Lives (years)" description="Straight-line per capex category">
        <Field label="Solar PV"                  name="depreciation.solarPV"              register={register} error={errors['depreciation.solarPV']}              suffix="years" />
        <Field label="Inverters & Accessories"   name="depreciation.invertersAccessories" register={register} error={errors['depreciation.invertersAccessories']} suffix="years" />
      ...
      {/* Tax */}
      <Section title="Tax" description="Applies after holiday ends and only when PBT ≥ 0">
      ...
      {/* Grant */}
      <Section title="Grant Assumptions" description="Optional capital grant">
      ...
      {/* Bridge financing */}
      <Section title="Bridge Financing" description="Funds project development; refinanced by senior debt">
        ...
        <Field label="Interest Pmt Frequency" name="financing.bridge.interestRepaymentFreqMonths" register={register} error={errors['financing.bridge.interestRepaymentFreqMonths']} suffix="months" />
      </Section>

      {/* Senior debt */}
      <Section title="Senior Debt (Refinancing)" description="Amortizing loan after operations start">
        ...
        <Field label="# Principal Repayments"  name="financing.senior.numberOfPrincipalRepayments"  register={register} error={errors['financing.senior.numberOfPrincipalRepayments']}  suffix="#" />
        ...
        <Field label="Interest Pmt Frequency"  name="financing.senior.interestRepaymentFreqMonths"  register={register} error={errors['financing.senior.interestRepaymentFreqMonths']}  suffix="months" />
      </Section>

      {/* Working capital */}
      <Section title="Working Capital" description="Days of receivables / payables">
      ...
      {/* Footer button - duplicate for long page */}
      <div className="flex justify-end">
        <Button type="submit" disabled={!isDirty || saveState === 'saving'} size="lg">
          <Save size={14} /> {saveState === 'saving' ? 'Saving…' : 'Save Assumptions'}
        </Button>
      </div>
    </form>
